function [T_sorted, best_design, results] = SLIT_spectrograph_sweep(varargin)
% SLIT_SPECTROGRAPH_SWEEP - Sweep slit-limited spectrograph parameters
%
...
%   'sampling_min'       - Minimum detector sampling [pixels/resolution element]
...
%   'save_results'       - Save sorted table to MAT file (default: true)
%   'output_file'        - MAT filename for saved results
%   'make_plot'          - Plot resolving power of highest-ranked design
%
% Outputs:
%   T_sorted     - Table of all valid configurations, feasible designs first
%   best_design  - Highest-ranked feasible design as a one-row table
%   results      - Structure array containing full wavelength-dependent data
%
% Example:
%   [T, best, results] = SLIT_spectrograph_sweep(...
%       'R_list', 1300:50:1500, ...
...
addParameter(p, 'sampling_min', 3, @(x) isnumeric(x) && isscalar(x) && x > 0);
addParameter(p, 'NA', 0.11, @(x) isnumeric(x) && isscalar(x) && x > 0 && x < 1);
addParameter(p, 'R_list', [1300, 1350, 1400, 1450, 1500], ...
    @(x) isnumeric(x) && all(x > 0));
addParameter(p, 'slit_widths', 50e-6, @(x) isnumeric(x) && all(x > 0));
addParameter(p, 'grating_densities', 1511e3, @(x) isnumeric(x) && all(x > 0));
...
addParameter(p, 'save_results', true, @(x) islogical(x) || ismember(x, [0, 1]));
addParameter(p, 'output_file', 'sweep_results.mat', @(x) ischar(x) || isstring(x));
addParameter(p, 'make_plot', true, @(x) islogical(x) || ismember(x, [0, 1]));
...
F1 = 1 / (2 * params.NA);                 % paraxial collimator f-number
...
for idet = 1:size(detectors, 1)
    ...
    pix = detectors{idet, 1} * 1e-6;       % detector pixel pitch [m]
    nx = detectors{idet, 2};
        ...
        for islit = 1:numel(s1_list)
            ...
            for irho = 1:numel(rho_list)
                ...
                for iga = 1:numel(geometries)
                    ...
                    % Check that the complete wavelength band can be diffracted.
                    ...
                    % Beam anamorphic ratio between camera and collimator sides.
                    Ma = cos(beta) / cos(alpha);

                    %% Camera and collimator geometry
                    s2_target = params.sampling_min * pix;

                    % Camera focal length required for the selected sampling
                    % at the central wavelength.
                    f2 = (s2_target * cos(beta)) / (rho * delta_lambda);

                    % With the projected grating geometry below, F2/F1 equals
                    % the slit-image magnification in the dispersion direction.
                    F2 = (s2_target / s1) * F1;
                    D2 = f2 / F2;
                    ...
                    % Focal-length ratio and actual slit-image magnification.
                    focal_magnification = f2 / f1;
                    slit_magnification = focal_magnification * cos(alpha) / cos(beta);

                    %% Dispersion and detector coverage
                    dlambda_dx = cos(beta) / (rho * f2);     % local dispersion [m/m]

                    % Exact focal-plane spectral length across the full band.
                    spectrum_length = abs(f2 * (tan(beta_max) - tan(beta_min)));
                    ...
                    %% Ranking heuristic
                    % Prefer greater detector use and a larger accepted slit.
                    ...
                    results(row).F2 = F2;
                    results(row).focal_magnification = focal_magnification;
                    ...
                    % Full arrays retained for plotting and further analysis.
...
for i = 1:n_display
    ...
    fprintf('%-5d %-15s %-8.1f %-6.1f %-6d %-7.0f %-8.0f | %-5.1f %-6.1f %-6.1f %-5.1f %-7.1f %-6.2f %-5.0f | %-5.2f %s\n', ...
        ...
        T_sorted.det_name{i}, ...
        T_sorted.pix_um(i), ...
        T_sorted.pixels_per_res(i), ...
        T_sorted.R(i), ...
        T_sorted.s1_um(i), ...
        T_sorted.rho_lpm(i), ...
        T_sorted.alpha_deg(i), ...
        T_sorted.f1_mm(i), ...
        T_sorted.f2_mm(i), ...
        T_sorted.F2(i), ...
        T_sorted.D2_mm(i), ...
        T_sorted.slit_magnification(i), ...
        T_sorted.n_pixels_range(i), ...
        T_sorted.FoM(i), ...
        feasible_char);
end
...
fprintf(' Focal ratio magnification=%.3f, slit-image magnification=%.3f\n', ...
    best_design.focal_magnification, ...
    best_design.slit_magnification);
...
fprintf('\nPerformance: %.1f px/res element, %.0f pixels (%.1f mm width) needed\n', ...
    best_design.pixels_per_res, ...
    best_design.n_pixels_range, ...
    detector_width_needed_mm);
...
if params.make_plot
    ...
    % config_id remains tied to the original structure-array index even after
